    ...
    const handleSearch = async (val: string) => {
        ...
        try {
            ...
            const filtrados = listaProductos
                ...
                .map((p: ProductoDTO) => {
                    const existente = actualesMarcados.find(m => m.idProducto === p.idProducto);
                    return existente ? existente : { ...p, cantidadLocal: 0, marcado: false };
                });
    ...
    const actualizarLocal = (id: number, cambios: Partial<ProductoConCantidad>) => {
        setResultados(prev => prev.map(p => {
            if (p.idProducto === id) {
                ...
                if (cambios.marcado === true && (p.cantidadLocal || 0) === 0) nuevo.cantidadLocal = 1;
                if (cambios.cantidadLocal !== undefined) nuevo.marcado = cambios.cantidadLocal > 0;
                if (nuevo.marcado && (nuevo.cantidadLocal || 0) > 0) onSelect({ ...nuevo, cantidad: nuevo.cantidadLocal! });
                return nuevo;
            }
            return p;
        }));
                ...
                <div className="space-y-4 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 bg-muted/50 p-2 rounded-lg h-11 px-3">
                            ...
                            />
                            <label htmlFor="checkSoloSel" className="text-[10px] font-bold uppercase cursor-pointer select-none">
                                Ver Marcados
                            </label>
                        </div>
                    </div>
                    <div className="max-h-[350px] overflow-y-auto border rounded-xl divide-y">
                        {resultadosVisibles.length > 0 ? resultadosVisibles.map((p) => (
                            <div key={p.idProducto} className="flex items-center justify-between p-3 hover:bg-muted/30 transition-colors">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => actualizarLocal(p.idProducto, { marcado: e.target.checked })}
                                    />
                                </div>
                            </div>
                        )) : (
