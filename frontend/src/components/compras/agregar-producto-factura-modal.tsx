...
// El catálogo general que recibe el modal sigue viniendo plano desde la API de productos
...
// Estructura ideal que espera el formulario de Facturas (alineado con Swagger)
export interface ProductoSeleccionadoParaFactura {
    idProducto: number;
    descripcion: string;
    categoria: string;
    idCategoria: number;
    ...
    cantidad: number;
    idFacturaCompraDetalle?: number; // Conservar para saber si viene de una edición existente
}

interface ProductoConSeleccion extends ProductoSeleccionable {
    cantidadSeleccionada: number;
    precioUnitarioEditado: number;
    marcado: boolean;
    idFacturaCompraDetalle?: number;
}

interface AgregarProductosFacturaModalProps {
    isOpen: boolean;
    onClose: () => void;
    productos: ProductoSeleccionable[];
    itemsExistentes: any[]; // Aquí viene directamente factura.detalles
    currentPage: number;
    totalPages: number;
    onPageChange: (page: number) => void;
    onCargarProductos: (productos: ProductoSeleccionadoParaFactura[]) => void;
    onNuevoProducto: () => void;
}

export function AgregarProductosFacturaModal({
    isOpen,
    onClose,
    productos,
    itemsExistentes,
    ...
    onCargarProductos,
    onNuevoProducto,
}: AgregarProductosFacturaModalProps) {
    ...
    // Memoria indexada por idProducto para retener selecciones entre páginas de la API
    ...
    // 1. Inicialización adaptada al esquema de Detalles de Factura de Compra
    useEffect(() => {
        if (!isOpen) return
        const mapaInicial = new Map()

        if (itemsExistentes && Array.isArray(itemsExistentes)) {
            itemsExistentes.forEach(item => {
                // Mapeo defensivo resolviendo las variantes del GET de facturas
                const idProd = item.idProducto || item.producto?.idProducto || item.id;
                if (!idProd) return

                const descFinal = item.producto?.descripcion || item.descripcion || "Producto sin descripción";
                const catFinal = item.producto?.categoria || item.categoria || "—";
                const precioFinal = item.precioUnitario !== undefined ? item.precioUnitario : (item.precio || 0);

                mapaInicial.set(idProd, {
                    id: idProd,
                    descripcion: descFinal,
                    categoria: catFinal,
                    idCategoria: item.idCategoria || item.producto?.idCategoria || 0,
                    marca: item.marca || item.producto?.marca || "—",
                    disponible: item.disponible !== undefined ? item.disponible : (item.producto?.disponible || 0),
                    cantidadSeleccionada: item.cantidad || 0,
                    precioUnitarioEditado: precioFinal,
                    marcado: true,
                    idFacturaCompraDetalle: item.idFacturaCompraDetalle // Guardamos la PK original del detalle
                })
            })
        }
    ...
    // 2. Sincronizar catálogo actual de la API con la memoria de seleccionados
    useEffect(() => {
        setProductosEstado(
            productos.map((p) => {
                ...
                return {
                    ...
                    precioUnitarioEditado: p.precio, // Por defecto toma el precio del catálogo base
                    marcado: false,
                }
            })
        )
    ...
    // 3. Manejador de cambios inline optimizado de forma unidireccional
    const actualizarProducto = (id: number, cambios: Partial<ProductoConSeleccion>) => {
        setMemoriaSeleccionados((prevMem) => {
            ...
            // Buscar si ya está en memoria o construirlo desde el estado visual actual
            const prodActual = nuevaMem.get(id) || productosEstado.find(p => p.id === id)

            if (!prodActual) return prevMem

            const nuevoEstado = { ...prodActual, ...cambios }

            // Lógica automática de Checkbox e Inputs
    ...
    // 4. Mapeo limpio de salida hacia el formulario de Editar Factura
    const cargarSeleccionados = () => {
        const seleccionados: ProductoSeleccionadoParaFactura[] = Array.from(memoriaSeleccionados.values())
            .map((p) => ({
                idProducto: p.id,
                descripcion: p.descripcion,
                categoria: p.categoria,
                idCategoria: p.idCategoria,
                precioUnitario: p.precioUnitarioEditado,
                cantidad: p.cantidadSeleccionada,
                idFacturaCompraDetalle: p.idFacturaCompraDetalle // Sigue viajando para no romper el PUT
            }))
            ...
            <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
                <DialogContent className="!w-[90vw] md:!w-[85vw] md:!max-w-5xl h-[92vh] max-h-[92vh] flex flex-col p-5 overflow-hidden gap-0">
                    <DialogHeader className="flex flex-row justify-between items-center space-y-0 pb-2">
                        <DialogTitle className="text-lg font-bold tracking-tight">Seleccionar Ítems de Factura</DialogTitle>
                        <Button
                            type="button"
                            ...
                            <Table className="table-fixed w-full">
                                <TableHeader className="bg-muted/50 sticky top-0 z-10 shadow-[0_1px_0_0_rgba(0,0,0,0.1)]">
                                    <TableRow className="h-9">
                                        ...
                                        <TableHead className="w-24 h-9 text-xs">Precio Unit.</TableHead>
                                ...
                                <TableBody>
                                    {productosVisibles.length === 0 ? (
                                        ...
                                        <>
                                            {productosVisibles.map((p) => {
                                                ...
                                                return (
                                                    <TableRow key={p.id} className={`h-[44px] max-h-[44px] transition-colors ${rowClass}`}>
                                                        ...
                                                        {/* Input inline para precio unitario (permite limpiar el campo para reescribir con fluidez) */}
                                                        <TableCell className="py-1">
                                                            <Input
                                                                className="w-20 h-6 text-left font-semibold text-xs px-1.5 bg-transparent border-dashed focus-visible:border-solid focus-visible:bg-background"
                                                                type="number"
                                                                min={0}
                                                                placeholder="0"
                                                                value={p.precioUnitarioEditado === 0 ? "" : p.precioUnitarioEditado}
                                                                onChange={(e) => {
                                                                    const val = e.target.value === "" ? 0 : Number(e.target.value)
                                                                    actualizarProducto(p.id, { precioUnitarioEditado: val })
                                                                }}
                                                            />
                                                        </TableCell>
                                            ...
                                            {/* Relleno estético de filas vacías para evitar saltos de layout */}
                    ...
                    {/* Footer del Modal con Paginación Integrada */}
                    <div className="flex flex-col sm:flex-row gap-3 justify-between items-center pt-2 shrink-0">
                        ...
                        <div className="flex gap-2 w-full sm:w-auto justify-end">
                            ...
                            <Button type="button" size="sm" className="h-8 text-xs shadow-sm" onClick={cargarSeleccionados}>
                                Cargar en Factura ({memoriaSeleccionados.size})
                            </Button>
                        </div>
                    </div>
                </DialogContent>
