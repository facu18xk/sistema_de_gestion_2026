...
export function SeleccionarItemsPedidoModal({
    ...
    useEffect(() => {
        ...
        const seleccionadosIds = new Set(itemsSeleccionados.map((i) => i.productoId || i.idProducto));
    ...
    const toggleSeleccion = (idProducto: number) => {
        setProductosEstado((prev) =>
            prev.map((p) => (p.idProducto === idProducto ? { ...p, marcado: !p.marcado } : p))
        );
    };
    ...
    const cantidadSeleccionadosTotales = productosEstado.filter((p) => p.marcado).length;
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="!w-[90vw] md:!w-[85vw] md:!max-w-4xl max-h-[92vh] flex flex-col p-5 overflow-hidden gap-0">
                        ...
                        <Table>
                            <TableHeader className="bg-muted/50">
                                <TableRow className="h-8">
                                    ...
                                    <TableHead className="h-8 py-0.5 text-xs max-w-[280px]">Descripción</TableHead>
                                    <TableHead className="h-8 py-0.5 text-xs">Categoría</TableHead>
                                    <TableHead className="w-24 text-center h-8 py-0.5 text-xs">Cant. Pedida</TableHead>
                                    <TableHead className="w-24 text-center h-8 py-0.5 text-xs">Incluir</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {productosVisibles.length === 0 ? (
                                    ...
                                    productosVisibles.map((p) => (
                                        <TableRow
                                            key={p.idProducto}
                                            className={`h-9 ${p.marcado ? "bg-primary/5 hover:bg-primary/10 transition-colors" : "hover:bg-muted/50"
                                                }`}
                                        >
                                            <TableCell className="font-mono text-[11px] text-muted-foreground py-0.5">
                                                #{p.idProducto}
                                            </TableCell>
                                            <TableCell className="font-medium text-xs py-0.5 max-w-[280px] truncate" title={p.descripcion}>
                                                {p.descripcion}
                                            </TableCell>
                ...
                <div className="flex gap-2 justify-end pt-2 border-t mt-2">
                    ...
                    <Button type="button" size="sm" className="h-8 text-xs shadow-sm" onClick={handleGuardar}>
